...
<c:set var="totalElements" value="${thispage.totalElements}" />
	...
	<ul class="pager text-right">
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<li class="disabled"><a href="#" title="${fn:escapeXml(last_label)}">Last</a></li>
			</c:otherwise>
		</c:choose>
	</ul>
	<spring:message code="list_size" var="list_size" text="Records per page" htmlEscape="false" />
	<c:out value="Found ${totalElements} records. ${list_size} " />
